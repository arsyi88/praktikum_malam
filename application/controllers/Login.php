        ...
        if($user){
            ...
            if (password_verify($pswd, $user['password'])) {
                ...
                if ($user['role'] == 'admin') {
                    ...
                    $this->_updateLastLogin($userid);
                    redirect('sekretaris/menu');
                }
            } else {
                //jika password salah
                $this->session->set_flashdata('message', '<div class="alert alertdanger" role="alert"> <b>Error :</b> Password Salah. </div>');
                redirect('/');
            }
        } else {
            //Jika user tidak terdaftar
            // echo "User Kadada";
            $this->session->set_flashdata('message', '<div class="alert alertdanger" role="alert"> <b>Error :</b> User Tidak Terdaftar. </div>');
            redirect('/');
        }
    }
